  ...
  stages {
    ...
    stage('Release docker image for development') {
      ...
      steps {
        ...
        sh(returnStdout: true, script: '''
          images=`docker images | grep entropypool | grep review-service | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep review-service | grep $tag
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh 'sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/review-service/k8s/01-review-service.yaml'
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          ...
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/review-service/k8s/01-review-service.yaml
